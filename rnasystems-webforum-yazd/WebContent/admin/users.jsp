...
<tr>
	...
	<%	if( (start+range) < userCount ) { %>
		<a href="users.jsp?range=<%=range%>&start=<%=(start+range)%>">Next <%= (start+(2*range) < userCount)?range:(userCount % range) %></a> &raquo;
	<%	} else { %>
		&nbsp;
	<%	} %>
	</td>
...
<tr bgcolor="#ffffff">
    ...
    <td align="center"><%=user.getProperty("ranking")%></td>
    <td align="center">
