...
<%
        ...
        String namelocale = ParamUtils.getParameter(request,"locale");
		if (yazdHome == null) {
	...
	<%	if( !error && setYazdHome ) {
			...
			if( !error ) {
				...
				PropertyManager.setProperty("DefaultLocale",namelocale);
				// redirect
...
<%	} %>
	...
	<form action="setup5.jsp" method="post">
	...
	<tr>
		<td><font size="-1">Yazd Locale:</font></td>
		<td><select name="locale" size="1">
                <%
                    for (int i=0; i<YazdLocale.supportedLocales.length; i++) {
                        Locale tl = YazdLocale.supportedLocales[i];
                 %>
                        <option value="<%=tl.getLanguage()%><%="".equals(tl.getCountry())?"":","+tl.getCountry()%><%="".equals(tl.getVariant())?"":","+tl.getVariant()%>"><%=tl.getDisplayLanguage()%> <%="".equals(tl.getDisplayCountry())?"":", "+tl.getDisplayCountry()%> <%="".equals(tl.getDisplayVariant())?"":", "+tl.getDisplayVariant()%>
                <%
                    }
                %>
                </select></td>
