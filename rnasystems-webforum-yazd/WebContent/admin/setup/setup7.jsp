...
<%!
	private String[] getPropertyNames( DbConnectionProvider conProvider ) {
		Enumeration enume = conProvider.propertyNames();
		Vector v = new Vector();
...
<%	// get parameters
	...
	if( createAdmin ) {
		if (! password.equals(confirmPassword)) {
            ...
            error=true;
            errorMessage = "email address is not provided";
        }
        else {
			Connection con = null;
			Statement stmt = null;
			...
			try {
				String sql = "insert into yazdUser(userID,username,passwordHash,name,email) "
					+ " values(?,?,?,?,?)";
				...
				if( !error ) {
					out.println( "ok");
					response.sendRedirect("setup8.jsp");
					return;
				}
			}
...
<ul>
	...
	<form action="setup7.jsp" mode="post">
...
<p>
If you have already created an administrator account, you can <a href="setup8.jsp">
skip this step</a>.
