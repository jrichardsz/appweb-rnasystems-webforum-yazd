...
<%	//////////////////////////////////////////////////////////////////
	...
	if( isSystemAdmin || isModerator ) {
		...
		if (isSystemAdmin || isModerator){
            ...
            node.addChild( new TreeLeaf("Move Thread",     "forumThread.jsp") );
		}
		node.setVisible(true);
	...
	// category tree
	if( isSystemAdmin ) {
            com.Yasna.forum.util.tree.Tree categoryTree
                    = new com.Yasna.forum.util.tree.Tree("category");
            int nodeID = 0;
            TreeNode node = null;

            node = new TreeNode( nodeID++, "Categories" );
            node.addChild( new TreeLeaf("Summary", "categories.jsp") );
            node.addChild( new TreeLeaf("Create", "createCategory.jsp") );
            node.addChild( new TreeLeaf("Edit Properties", "editCategory.jsp") );
            node.addChild( new TreeLeaf("Remove", "removeCategory.jsp") );
            node.addChild( new TreeLeaf("Content",     "categoryContent.jsp") );
            node.setVisible(true);
            categoryTree.addChild(node);

            node = new TreeNode( nodeID++, "ForumGroups" );
            node.addChild( new TreeLeaf("Summary", "forumGroups.jsp") );
            node.addChild( new TreeLeaf("Create", "createForumGroup.jsp") );
            node.setVisible(true);
            categoryTree.addChild(node);

            adminBean.addTree( "categoryTree", categoryTree );
	}
%>
